...
Interpreter.prototype._callbackStep = function (thisStep) {
  if (typeof this.stepListener === 'function') {
    try {
      this.stepListener(thisStep, cloneDeep(this.stack.rawstack, true), cloneDeep(this.altstack.rawstack, true))
    } catch (err) {
 ...
 * @param {*} stack
 */
Interpreter.prototype._updateStack = function (stack, pc, scriptType) {
  if (typeof this.stackListener === 'function') {
    try {
      this.stackListener(stack, pc, scriptType)
    } catch (err) {
      console.log(`Error in _updateStack:${err}`)
    }
  }
}
...
Interpreter.prototype.step = function (scriptType) {
  ...
  if (fExec) {
    this._updateStack(this.stack, this.pc - 1, scriptType)
  }
  ...
  if (fExec && opcodenum >= 0 && opcodenum <= Opcode.OP_PUSHDATA4) {
    ...
    switch (opcodenum) {
      ...
      case Opcode.OP_ROT:
        ...
        this.stack.updateVarsStack([var2, var3, var1])
        break
      ...
      case Opcode.OP_SWAP:
        ...
        this.stack.write(-1, x1)
        this.stack.updateVarsStack([var2, var1])
        break
      ...
      case Opcode.OP_TUCK:
        ...
        this.stack.updateVarsStack([var2, var1, `$${var2}`])
        break
