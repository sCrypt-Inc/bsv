...
Transaction.prototype.toTxHashPreimageBuffer = function () {
  ...
  _.each(this.inputs, function (input) {
    input.toTxHashPreimageWriter(writer)
  })
  writer.writeVarintNum(this.outputs.length)
  _.each(this.outputs, function (output) {
    output.toTxHashPreimageWriter(writer)
  })
...
Transaction.prototype.getSignatures = function (privKey, sigtype) {
  ...
  // By default, signs using ALL
  sigtype = sigtype || Signature.SIGHASH_ALL
  var transaction = this
  var results = []
  _.each(this.inputs, function forEachInput (input, index) {
    _.each(input.getSignatures(transaction, privKey, index, sigtype), function (signature) {
