...
function getLowSPreimage (tx, sigtype, inputIndex) {
  var i = 0
  do {
...
Input.prototype._fromObject = function (params) {
  ...
  this.output = params.output
    ? Output.fromObject(params.output)
    : undefined
...
Input.prototype.toTxHashPreimageWriter = function (writer) {
  if (!writer) {
    writer = new BufferWriter()
  }
  writer.writeReverse(this.prevTxId)
...
Input.prototype.getSignatures = function (transaction, privateKeys, inputIndex, sigtype) {
  ...
  if (privateKeys instanceof PrivateKey) {
    results.push(new TransactionSignature({
      ...
      signature: Sighash.sign(transaction, privateKeys, sigtype, inputIndex),
      sigtype: sigtype
    }))
    ...
    _.each(privateKeys, function (privateKey, index) {
      ...
      results.push(new TransactionSignature({
        ...
        inputIndex: inputIndex,
        signature: Sighash.sign(transaction, privateKey, sigtype_, inputIndex),
        sigtype: sigtype_
      }))
    })
  }
